<?php
	...
	{
		$id = $_GET['edit_id'];
		$stmt_edit = $DB_con->prepare('SELECT title, caption, img FROM tbl_amenities3 WHERE userID =:uid');
	...
	if(isset($_POST['btn_save_updates']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE tbl_amenities3
									     SET title=:tit, 
										     caption=:cap, 
										     img=:img 
								       WHERE userID=:uid');
			...
			if($stmt->execute()){
				...
				window.location.href='uploada3.php';
				</script>
                <?php
			}
			else{
