<?php
	...
	{
		$id = $_GET['edit_id'];
		$stmt_edit = $DB_con->prepare('SELECT title, caption, img FROM tbl_users3 WHERE userID =:uid');
	...
	if(isset($_POST['btn_save_updates']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE tbl_users3 
									     SET title=:tit, 
										     caption=:cap, 
										     img=:img 
								       WHERE userID=:uid');
			...
			if($stmt->execute()){
				...
				window.location.href='upload3.php';
				</script>
                <?php
			}
			else{
...
<form method="post" enctype="multipart/form-data" class="form-horizontal">
    ...
    <tr>
        ...
        <td><input class="form-control" type="text" name="user_name" value="<?php echo $title; ?>" required /></td>
    </tr>
    
    <tr>
    	<td><label class="control-label">Caption</label></td>
        <td><input class="form-control" type="text" name="user_job" value="<?php echo $caption; ?>" required /></td>
    </tr>
