  <?php
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT img FROM tbl_amenities WHERE userID =:uid');
		$stmt_select->execute(array(':uid'=>$_GET['delete_id']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("user_images/".$imgRow['img']);
		
		// it will delete an actual record from db
		$stmt_delete = $DB_con->prepare('DELETE FROM tbl_amenities WHERE userID =:uid');
		$stmt_delete->bindParam(':uid',$_GET['delete_id']);
		$stmt_delete->execute();
		
		header("Location: uploada.php");
	}
...
?>
    ...
    <ul class="nav nav-tabs">
          ...
          <li role="presentation" class="active"><a href="uploada.php">Chairs/Tables</a></li>
	...
	<div class="page-header">
    	<h1 class="h2">&nbsp; &nbsp;UPLOAD EVENTS / &nbsp; &nbsp;<a class="btn btn-default" href="addamenities.php"> <span class="glyphicon glyphicon-plus"></span>  Upload </a></h1> 
    </div>
...
<?php
	...
	$stmt = $DB_con->prepare('SELECT userID, title, caption, img FROM tbl_amenities ORDER BY title ASC');
	$stmt->execute();
		...
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			<div class="col-xs-6">
              <br>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  
				...
				<a class="btn btn-info" href="editforma.php?edit_id=<?php echo $row['userID']; ?>" title="click for edit" >
